...
# Before running, ensure your EC2 instance has an IAM role attached with the
# following permissions so the collector can read instance tags (e.g. the Name tag):
#
#   ec2:DescribeTags
#   ec2:DescribeInstances
#
# Without this role, resource detection will fall back to hostname only and
# the EC2 instance Name tag will not appear in your metrics and logs.
...
if ! id -u openobserve-agent &>/dev/null; then
    ...
    usermod -aG systemd-journal openobserve-agent
fi
...
processors:
  ...
  # Requires the EC2 instance to have an IAM role with ec2:DescribeTags and ec2:DescribeInstances permissions.
